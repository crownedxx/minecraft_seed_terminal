...
export function parseAllMatches(output: string): SearchMatch[] {
  ...
  for (const line of output.split("\n")) {
    ...
    if (!inTable) continue;
    const row = line.match(/^\s*(\d+)\s+(-?\d+)\s+(-?\d+)\s+([\d.]+)\s+\((-?\d+),\s*(-?\d+)\)\s*$/);
    if (row) {
      matches.push({
        id: Number(row[1]),
        x: Number(row[2]),
        ...
        chunk: { x: Number(row[5]), z: Number(row[6]) },
      });
    }
  }
  return matches;
...
export const HELP_TEXT = `Available commands:
  ...
  copy [number] <match number>    Copy a biome/structure match as /tp
  copy branch <branch> number <vein>    Copy a vein's coords as /tp command
